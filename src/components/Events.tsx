        ...
        {loading ? (
          ...
          <div className={`grid gap-8 ${events.length === 1
            ...
            {events.map((event) => (
              <div
                key={event.id}
                className={`bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group border border-gray-100 ${events.length === 1
              ...
              >
                ...
                <div className={`relative overflow-hidden ${events.length === 1
                  ...
                  {event.image_url ? (
                    ...
                    <div className={`w-full h-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center ${events.length === 1
                        ? 'lg:h-96'
                        : ''
                      }`}>
                ...
                <div className={`p-6 ${events.length === 1 ? 'lg:p-10 lg:flex lg:flex-col' : ''}`}>
                  <div className={`flex justify-between items-start mb-3 ${events.length === 1 ? 'lg:mb-6' : ''}`}>
                    <h3 className={`font-bold text-gray-900 group-hover:text-blue-600 transition-colors ${events.length === 1
                    ...
                    <div className="text-right">
                      <p className={`font-semibold text-blue-600 ${events.length === 1
                          ? 'text-lg lg:text-xl'
                          : 'text-sm'
                        }`}>{event.time}</p>
                    </div>
                  ...
                  <div className={`${events.length === 1 ? 'lg:mt-auto' : ''}`}>
                    <Link href={`/tickets?event=${event.id}`}>
                      <Button
                        size="sm"
                        className={`bg-blue-600 hover:bg-blue-700 transition-all duration-300 ${events.length === 1
