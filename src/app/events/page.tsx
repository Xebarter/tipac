      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            Join us for our upcoming theatre performances, workshops, and
            cultural events.
          </p>
        </motion.div>
          ...
          >
            {events.map((event, index) => {
              ...
              return (
                <motion.div
                  key={event.id}
                  className={`rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group backdrop-blur-sm bg-white/70 border border-gray-100 ${
                    events.length === 1
                ...
                >
                  ...
                  >
                    {event.image_url ? (
                      <Image
                        src={
                          event.image_url.startsWith("http")
                            ? event.image_url
                            : `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/events/${event.image_url}`
                        }
                        alt={event.title}
                        fill={true}
                      ...
                      <div
                        className={`w-full h-full bg-gradient-to-r from-purple-500 to-pink-600 flex items-center justify-center ${
                          events.length === 1 ? "lg:h-96" : ""
                        }`}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div className="absolute top-4 right-4">
                      <span
                        className={`inline-block px-3 py-1 text-xs font-semibold text-white bg-gradient-to-r ${colorScheme.badge} rounded-full backdrop-blur-sm shadow`}
                      >
                  ...
                  {/* Event Details */}
                  <div
                    className={`p-6 ${events.length === 1 ? "lg:p-10 lg:flex lg:flex-col" : ""}`}
                  >
                    <div
                      className={`flex justify-between items-start mb-3 ${events.length === 1 ? "lg:mb-6" : ""}`}
                    >
                      <h3
                    ...
                    <div
                      className={`${events.length === 1 ? "lg:mt-auto" : ""}`}
                    >
                      <Link href={`/tickets?event=${event.id}`}>
                        <Button
                          size="sm"
                          className={`bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 font-medium rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 ${
                            events.length === 1
                    ...
                    {event.sponsor_logos && event.sponsor_logos.length > 0 && (
                      <div className="mt-6 pt-4 border-t border-gray-200">
                        ...
                        <div className="flex flex-wrap justify-between items-center gap-4">
                          {event.sponsor_logos.map((sponsor, logoIndex) => (
                            <div
                              key={logoIndex}
                              className="relative w-20 h-12 bg-white rounded-lg shadow-sm overflow-hidden flex items-center justify-center p-2 flex-grow"
                            >
                              <Image
                                src={sponsor.url}
                                alt={sponsor.name}
