...
export default function InvitationCardScannerPage() {
  const [result, setResult] = useState<{ valid: boolean; message: string; card?: any } | null>(null);
  ...
  const extractCardId = (rawValue: string) => {
    if (!rawValue) {
      return { error: "Please enter a card ID" } as const;
    }
    ...
    if (parsedJson && typeof parsedJson === "object") {
      const cardIdFromJson = (parsedJson as Record<string, unknown>).card_id ?? (parsedJson as Record<string, unknown>).id;
      if (typeof cardIdFromJson === "string" && cardIdFromJson.trim()) {
        return { cardId: cardIdFromJson.trim() } as const;
      }
    }

    if (trimmed.includes("card_id")) {
      const match = trimmed.match(/[\"']?card[_-]?id[\"'\s:=]+([0-9a-fA-F-]{8,})/i);
      if (match && match[1]) {
        return { cardId: match[1] } as const;
      }

      try {
        const params = new URLSearchParams(trimmed);
        const fromParams = params.get("card_id");
        if (fromParams && fromParams.trim()) {
          return { cardId: fromParams.trim() } as const;
        }
      } catch (err) {
    ...
    if (!sanitized) {
      return { error: "Unable to extract card ID from QR data" } as const;
    }

    return { cardId: sanitized } as const;
  };
  ...
  const verifyCard = async (id: string) => {
    try {
      ...
      const response = await fetch(`/api/invitation-cards/verify/${encodeURIComponent(normalizedId)}`);
      ...
      setError("Failed to verify invitation card");
      console.error(err);
    }
  };
  ...
  const markAsUsed = async () => {
    ...
    try {
      const response = await fetch(`/api/invitation-cards/verify/${result.card.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (data.valid) {
        setResult({
          ...
          message: "Invitation card marked as used successfully"
        });
        setRecentlyMarkedUsedId(result.card.id);
        setSuccessMessage("Invitation card marked as used successfully");
        if (successTimeoutRef.current) {
          clearTimeout(successTimeoutRef.current);
        }
        ...
        setError(data.message || "Failed to mark invitation card as used");
        return false;
      }
    } catch (err) {
      setError("Failed to mark invitation card as used");
      console.error(err);
      return false;
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            Scan QR codes or manually enter card IDs to verify authenticity
          </p>
        </div>

        {successMessage && (
          <div className="max-w-2xl mx-auto mb-8">
            <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-green-700 text-center">
              {successMessage}
            </div>
          </div>
        )}
        ...
        <div className="max-w-2xl mx-auto bg-white/70 backdrop-blur-sm rounded-2xl border border-gray-100 p-6 shadow-sm">
          ...
          <p className="text-gray-600 text-sm">
            Enter a card ID manually if you can't scan the QR code
          </p>
        </div>
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-bold text-gray-900">
                    {result.valid ? "Valid Invitation Card" : "Invalid Invitation Card"}
                  </h3>
                  <button 
                    onClick={closePopup}
                ...
                {result.valid && result.card ? (
                  <div className="space-y-4">
                    <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 border border-purple-100">
                      <h4 className="font-bold text-gray-900 mb-2">{result.card.event?.title}</h4>
                      <div className="space-y-2 text-sm text-gray-700">
                        <p><span className="font-medium">Card ID:</span> {result.card.id.substring(0, 8)}...</p>
                        <p><span className="font-medium">Type:</span> {result.card.card_type}</p>
                        <p><span className="font-medium">Event Date:</span> {result.card.event?.date ? new Date(result.card.event.date).toLocaleDateString() : 'N/A'}</p>
                        <p><span className="font-medium">Location:</span> {result.card.event?.location || 'N/A'}</p>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center">
                        {result.card.is_used ? (
                          ...
                          <>
                            ...
                            <span className="font-medium text-green-700">Valid Card</span>
                          </>
                        )}
                      </div>
                    </div>
