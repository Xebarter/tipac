...
export default function AdminGalleryManagement() {
  ...
  const loadImages = useCallback(async () => {
    try {
      ...
      setUploadStatus({ message: "Error loading images. Please refresh.", isError: true });
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            ...
            <dd className="mt-1 text-3xl font-semibold text-gray-900">{images.length}</dd>
          </div>
        </div>
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            ...
            <dd className="mt-1 text-3xl font-semibold text-gray-900">
              {images.length > 0 ? new Date(images[0].created_at).toLocaleDateString() : 'N/A'}
            </dd>
          </div>
        </div>
      </div>
      ...
      {/* Upload Area */}
      {uploadStatus && (
        <div className={`mb-6 p-4 rounded-md ${uploadStatus.isError ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
      ...
      <div className="bg-white shadow-sm rounded-lg overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-6">
            {images.map((image) => (
              <div key={image.id} className="group relative bg-white rounded-lg overflow-hidden border border-gray-200 hover:shadow-md transition-all duration-200">
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="ml-2 flex-shrink-0 flex">
                      ...
                      <button
                        onClick={() => handleDeleteImage(image.id, image.original_name)}
