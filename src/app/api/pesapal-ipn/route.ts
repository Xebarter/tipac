...
export async function POST(req: NextRequest) {
  try {
    ...
    if (OrderNotificationType === "PAYMENT_COMPLETED") {
      ...
      console.log(
        `Successfully updated ticket status for OrderTrackingId: ${OrderTrackingId}`,
      );
    } else if (
      OrderNotificationType === "PAYMENT_FAILED" ||
      OrderNotificationType === "PAYMENT_CANCELLED"
    ) {
      console.log(
        `Payment ${OrderNotificationType.toLowerCase()} for OrderTrackingId: ${OrderTrackingId}`,
      );
